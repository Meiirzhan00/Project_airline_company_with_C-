...
namespace OOP_PROJECT
{
    class Contact_information  // Класс жұмысы : бұл класта клиенттер туралы ақпарат сақталады.
    {
        ...
        public string sAddress;    // Компания мекен-жайы, яғни затты алып кететін орын(склад).
        public string dAddress;    // Тапсырыс жеткізілетін орын мекен-жайы. 
        ...
        public Contact_information()    // Бос конструктор
        {
            ...
            Console.Write("\nАдрес получения : ");
            this.dAddress = Console.ReadLine();
        ...
        public void Shygaru()
        {
            Console.WriteLine($"Компания : {company}  Имя : {name}  Email : {email} {sAddress}");
        }
    ...
    class PressKey    // Бұл клавишті басқанда орындалатын оқиға
    {
        public delegate void KeyDelegate();
        public event KeyDelegate yKeyEvent = null;
        public event KeyDelegate nKeyEvent = null;
        public void YKeyEvent()    //  Мұнда клиент  y  клавишін басса осы оқиға орындалады
        {
            if (yKeyEvent != null)
            {
                yKeyEvent.Invoke();
        ...
        public void NKeyEvent()   //  Мұнда клиент  n клавишін басса осы оқиға орындалады
        {
            if (nKeyEvent != null)
            {
                nKeyEvent.Invoke();
    ...
    class TimeSay   //  Мұнда белгілі бір уақыт өткен соң клиент заказының қай жерде тұрғанын хабарлайды.
    {
        public delegate void timeDel();
        public event timeDel timeEvent;
        public void ProTimeEvent()
        {
            if (timeEvent != null)
            {
                timeEvent.Invoke();
    ...
    class MessTime
    {
        public void TimeS()
        {
            Console.Write("Ваш заказ здесь : ");
        }
    }
    class KeyEvents
    {

        public void Y_Click()    //  y клавишін басқанда оқиға төмендегі орындалады. Клиентке өз затының маршруты хабарлама ретінде келеді.
        {                      
            DateTime d = new DateTime();
            Console.WriteLine("\nСтатус маршрута !!!");
            Console.WriteLine($"-- 1. Konaev1 -- 2. Kazakhstan -- 3. Brazil -- 4. Utmakhanbet3");
            Console.WriteLine($"Ваш заказ здесь : 1. Konaev1");
            Console.WriteLine($"Ваш заказ здесь : 2. Kazakhstan");
            Console.WriteLine($"Ваш заказ здесь : 3. Brazil");
            Console.WriteLine($"Ваш заказ здесь : 4. Utmakhanbet3");
    ...
    class CargoEvents   // Бұл оқиға клиент тапсырыс беруді бастаған кезде орындалады. Оған заказ берудің басталғанын және соңында аяқталғанын хабарлайды.
    {
        public delegate void Process();
        public event Process StartEndProcess;
        Program t = new Program();
        public void ProcessEvent()
        {
            if (Program.tanda == 1)
            {
                if (StartEndProcess != null)
                {
                    StartEndProcess();
                }
            }
    ...
    class ProcessEvents
    {
        public void HundlerSE()   // Клиент заказ беруді бастағанда оған заказ берудің басталғаны туралы оқиға хабарланады.
        { 
            Console.WriteLine("СООБЩАЮ О НАЧАЛЕ РАБОТЫ ПО ЗАКАЗУ !");
        }

        public void HundlerSE2()   // Клиент заказ беруді аяқтағанда оған заказ берудің аяқталғаны туралы оқиға хабарланады.
        {
            Console.WriteLine("СООБЩАЮ О ЗАВЕРШЕНИИ РАБОТ ПО ЗАКАЗУ !");
        }
    }
    class Program
    {
        public static int tanda;
        static void Main(string[] args)
        {
            CargoEvents c = new CargoEvents();
            ...
            while (tr)
            {
                ...
                tanda = Convert.ToInt32(Console.ReadLine());
                        ...
                        case 1:
                            ...
                            while (t)
                            {
                                ...
                                if (selectF == 1)
                                {
                                    ...
                                    if (selectP == 1)
                                    {
                                        PressKey kM = new PressKey();
                                        KeyEvents kE = new KeyEvents();

                                        kM.yKeyEvent += kE.Y_Click;
                                        kM.nKeyEvent += kE.N_Click;

                                        Console.WriteLine("\nСлужба выполнена успешно. Спасибо за доверие к нашей компании !\n");
                                        c.StartEndProcess -= p.HundlerSE;
                                        c.StartEndProcess -= p.HundlerSE;
                                        c.StartEndProcess += p.HundlerSE2;
                                        c.ProcessEvent();

                                        ConsoleKey pressedKey;
                                        Console.Write("\nХотите, чтобы вам сообщали маршрут доставки груза ? [Y/N] :");
